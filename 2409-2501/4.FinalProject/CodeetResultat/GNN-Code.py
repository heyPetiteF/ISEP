...
import pandas as pd
from tensorflow.keras.utils import Progbar  # Keras progress bar replacement
...
def normalizeY(longitude_arr, latitude_arr):
    ...
    print("Before Normalization - Latitude:", latitude_arr[:10])
    ...
    print("After Normalization - Latitude:", latitude_arr[:10])
    return longitude_arr, latitude_arr

def reverse_normalizeY(longitude_arr, latitude_arr):
    global longitude_scaler, latitude_scaler
    print("\nBefore Reverse Normalization - Longitude:", longitude_arr[:10])
    print("Before Reverse Normalization - Latitude:", latitude_arr[:10])
    ...
    print("After Reverse Normalization - Latitude:", latitude_arr[:10])
    return longitude_arr, latitude_arr

def plot_histogram(df_X, save_path):
    print("\nVisualizing and Saving RSSI Data Distribution...")
    num_columns = min(100, len(df_X.columns))
    fig, axes = plt.subplots(10, 10, figsize=(20, 15))
    axes = axes.flatten()

    for i in range(num_columns):
        df_X.iloc[:, i].hist(bins=30, ax=axes[i])
        axes[i].set_title(f"Feature {i}", fontsize=8)
        axes[i].tick_params(axis='x', labelsize=6)
        axes[i].tick_params(axis='y', labelsize=6)

    plt.tight_layout()
    save_file = os.path.join(save_path, "RSSI_Test_b0.png")
